...
export function AdminReportDetailPage() {
  ...
  const reload = useCallback(() => {
    setLoading(true);
    getAdminReport(id!).then(setReport).catch(() => {}).finally(() => setLoading(false));
  }, [id]);
  ...
  const handleApprove = async () => {
    setActionError('');
    try { await approveReport(id!); reload(); }
    catch { setActionError('Failed to approve'); }
  };

  const handleReject = async () => {
    setActionError('');
    try { await rejectReport(id!); reload(); }
    catch { setActionError('Failed to reject'); }
  };

  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
      <button onClick={() => navigate('/admin/reports')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6', marginBottom: 16 }}>
        ← Back
      </button>
      ...
      <p><strong>Total: ${Number(report.totalAmount).toFixed(2)}</strong></p>
      ...
      {isSubmitted && (
        <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
          <button onClick={handleApprove} style={{ background: '#22c55e', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 4, cursor: 'pointer' }}>
            Approve
          </button>
          <button onClick={handleReject} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 4, cursor: 'pointer' }}>
            Reject
          </button>
        </div>
      )}
